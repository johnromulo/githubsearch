...
export const gitReducer = (state = INITIAL_STATE, action) => {
  ...
  switch (action.type) {
    case env.GIT_SUCCESS_USER:
      return { user: action.data, repos: state.repos, loading: false, erros: false }
    case env.GIT_FAILURE_USER:
      return { user: {}, repos: state.repos, loading: false, erros: true };
    ...
    case env.GIT_SUCCESS_USER_REPOS:
      ...
      return { user: userNew, repos: action.data.sort(oderStar), loading: false, erros: false }
    case env.GIT_FAILURE_USER_REPOS:
      return { user: state.user, repos: [], loading: false, erros: true };
